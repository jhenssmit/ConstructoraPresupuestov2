...
$formularioDatos1 = isset($_COOKIE['formularioDatos1']) ? json_decode($_COOKIE['formularioDatos1'], true) : array();
...
if(isset($_POST['submit'])) {
    ...
    $formularioDatos1[] = $nuevoDato;

    // guardar los datos en la cookie y localStorage
    setcookie('formularioDatos1', json_encode($formularioDatos1), time() + 3600, '/');
    echo "<script>localStorage.setItem('formularioDatos1', JSON.stringify(" . json_encode($formularioDatos1) . "));</script>";
}
...
$formularioHTML = '<form id="formularioDatos" method="post" action="http://localhost/constructora/Views/historialTecho.php">';
  //bucle de los datos
foreach ($formularioDatos1 as $key => $data) {
...
<ol class="breadcrumb mb-4">
    <li class="breadcrumb-item active">Presupuesto de Materiales que van en el Techo</li>
</ol>
<?php if (empty($formularioDatos1)): ?>
...
<?php else: ?>
  <table class="table table-light">
    ...
    <tbody>
      <?php foreach ($formularioDatos1 as $key => $data): ?>
        <tr>
          <td><?php echo $data['nombre']; ?></td>
          <td>S/ <?php echo $data['precioBajo']; ?></td>
          <td>S/ <?php echo $data['precioMedio']; ?></td>
          <td>S/ <?php echo $data['precioAlto']; ?></td>
          <td><button class="btn btn-danger" onclick="eliminarFila1('<?php echo $data['nombre']; ?>')">Eliminar</button></td>
        </tr>
      <?php endforeach; ?>
    </tbody>
  </table>
<?php endif; ?>
<form action="PresupuestoTecho" method="post">
